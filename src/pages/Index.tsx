...
const Index = () => {
  ...
  const crop = banner.imageMeta?.crop || {
    x: 0,
    y: 0
  };
  const zoom = banner.imageMeta?.zoom || 1;
  return <div className="px-4 space-y-6 animate-fade-in">
      {/* Banner Principal Personalizável */}
      {banner.isVisible && <div className="relative overflow-hidden rounded-3xl glass-card p-6 text-center min-h-[200px]">
          {bgUrl && <div className="absolute inset-0 overflow-hidden rounded-3xl">
              <img src={bgUrl} alt="Banner Background" className="w-full h-full object-cover opacity-30" style={{
          ...
          <div className="relative z-10">
            <div className="w-24 h-24 mx-auto mb-4 rounded-full gradient-gold p-1">
              <div className="w-full h-full rounded-full bg-salon-dark flex items-center justify-center">
                {logoUrl ? <img src={logoUrl} alt="Logo" className="w-20 h-20 object-contain rounded-full" /> : <img src="/lovable-uploads/6c513fb2-7005-451a-bfba-cb471f2086a3.png" alt="Marcos Mariano Logo" className="w-20 h-20 object-contain rounded-full" />}
              </div>
            </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Button onClick={() => navigate('/agendamento')} className="h-20 bg-gradient-to-r from-salon-gold to-salon-copper hover:from-salon-copper hover:to-salon-gold text-salon-dark font-semibold rounded-2xl flex flex-col space-y-1">
          <Calendar size={24} />
          <span>Agendar Horário</span>
        </Button>
        
        <Button onClick={() => navigate('/loja')} variant="outline" className="h-20 border-salon-gold text-salon-gold hover:bg-salon-gold/10 rounded-2xl flex flex-col space-y-1">
          <ShoppingBag size={24} />
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {testimonials.map((testimonial, index) => <Card key={index} className="glass-card border-salon-purple/20">
              <CardContent className="p-4">
                <div className="flex items-center space-x-1 mb-2">
                  {[...Array(testimonial.rating)].map((_, i) => <Star key={i} size={16} className="text-salon-gold fill-current" />)}
                </div>
