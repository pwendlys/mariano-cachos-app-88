...
const ProfessionalAvatarUpload: React.FC<ProfessionalAvatarUploadProps> = ({
  currentAvatarUrl,
  professionalId,
  professionalName,
  onAvatarUpdate
  ...
  const uploadAvatar = async (file: File) => {
    try {
      ...
      // Gerar nome único para o arquivo baseado no ID do profissional
      const fileExt = file.name.split('.').pop() || 'jpg';
      const fileName = `professionals/${professionalId}/avatar_${Date.now()}.${fileExt}`;
  ...
  const removeAvatar = async () => {
    try {
      ...
      // Atualizar na tabela profissionais (remover URL)
      const { error: updateError } = await supabase
        .from('profissionais')
        .update({ avatar: null })
        .eq('id', professionalId);
  ...
  return (
    <div className="flex flex-col items-center space-y-4">
      <div className="relative">
        <Avatar className="w-24 h-24 border-2 border-salon-gold">
          ...
          <AvatarFallback className="bg-salon-gold/20 text-salon-gold text-2xl">
            {professionalName?.charAt(0)?.toUpperCase() || <User size={32} />}
          </AvatarFallback>
        </Avatar>
      ...
      <div className="flex flex-col items-center space-y-2">
        <label htmlFor="professional-avatar-upload">
          <Button
            asChild
            variant="outline"
            size="sm"
...
export default ProfessionalAvatarUpload;
