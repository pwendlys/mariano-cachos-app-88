...
-- First, let's fix the commission trigger to handle NULL values properly
...
-- Also fix the cash flow trigger to handle NULL values
...
BEGIN
  ...
  IF NEW.status IN ('confirmado', 'concluido') AND 
    ...
    valor_final := COALESCE(NEW.valor, servico_preco, 0);
...
-- Make sure appointments have a valor field when completing
-- Add a trigger to ensure appointments have proper valor before marking as concluido
...
DECLARE
  servico_preco NUMERIC;
BEGIN
  -- Se estamos marcando como concluído, garantir que tem valor
  IF NEW.status = 'concluido' AND (OLD.status IS NULL OR OLD.status != 'concluido') THEN
    ...
    IF NEW.profissional_id IS NULL THEN
      RAISE EXCEPTION 'Não é possível marcar agendamento como concluído sem profissional atribuído';
    END IF;
    
    -- Validar se tem valor válido
...
-- Create the trigger for validation
